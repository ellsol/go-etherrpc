...
func TestFunctionSignature_ReadHead(t *testing.T) {
	hs, _ := ethertypes.NewHexString().SetString(inputDeliverToken)
...
func TestFunctionSignature_DecodeFunctionInputWithDeliverTokens(t *testing.T) {
	...
	// check content
	expectedAddress, _ := new(ethertypes.EtherAddress).SetString("0x499d4aab8acab015d319ebfe476e4c800079ce87")
	if err := CompareEtherAddress(expectedAddress, param0.Value.(*ethertypes.EtherAddress)); err != nil {
		t.Error(err)
		return
	}
...
func CompareEtherAddress(expected *ethertypes.EtherAddress, actual *ethertypes.EtherAddress) error {
