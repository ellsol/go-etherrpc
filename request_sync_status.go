...
func (ssr *SyncStatusRaw) ToSyncStatus() (*SyncStatus, error) {
	...
	startingBlock, err := ethertypes.NewHexString().SetString(ssr.StartingBlock)
	if err != nil {
		return nil, err
	}
	highestBlock, err := ethertypes.NewHexString().SetString(ssr.HighestBlock)
	if err != nil {
		return nil, err
	}
	currentBlock, err := ethertypes.NewHexString().SetString(ssr.CurrentBlock)
	if err != nil {
		return nil, err
	}
