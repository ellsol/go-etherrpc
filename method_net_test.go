...
func TestNet_NetListening(t *testing.T) {
	listening, err := NewRPCClient(config().url).Net.NetListening()
...
func TestNet_NetPeerCount(t *testing.T) {
	count, err := NewRPCClient(config().url).Net.NetPeerCount()
...
func TestNet_NetVersion(t *testing.T) {
	expectedVersion := "1"
	version, err := NewRPCClient(config().url).Net.NetVersion()
